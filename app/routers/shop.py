...
def checkout(
    ...
    try:
        ...
        request.session["last_customer_phone"] = payload["phone"]
        request.session["last_customer_email"] = payload["email"]
...
def my_orders(request: Request, db: Session = Depends(get_db), phone: str | None = None, email: str | None = None):
    orders = []
    phone = phone or request.session.get("last_customer_phone")
    email = email or request.session.get("last_customer_email")
    if phone or email:
    ...
    return templates.TemplateResponse("shop/my_orders.html", {"request": request, "orders": orders, "phone": phone or "", "email": email or ""})
