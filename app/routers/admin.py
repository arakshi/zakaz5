...
def dashboard(request: Request, db: Session = Depends(get_db)):
    require_role(request, ["admin", "catalog_manager", "florist", "delivery_manager"])
...
def orders_page(request: Request, db: Session = Depends(get_db)):
    require_role(request, ["admin", "florist", "delivery_manager"])
...
def assign_courier(order_id: int, request: Request, courier_id: int = Form(...), db: Session = Depends(get_db)):
    require_role(request, ["admin", "delivery_manager"])
...
def products_page(request: Request, db: Session = Depends(get_db)):
    require_role(request, ["admin", "catalog_manager"])
