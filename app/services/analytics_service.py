...
def dashboard_metrics(db: Session) -> dict:
    ...
    return {
        ...
        "statuses": _rows_to_pairs(statuses),
        "payment_methods": _rows_to_pairs(payment_methods),
        "pickup_methods": _rows_to_pairs(pickup_methods),
        "top_categories": _rows_to_pairs(top_categories),
        "top_products": _rows_to_pairs(top_products),
        "low_stock": [[name, int(qty or 0)] for name, qty in low_stock],
        "conversion": {"views": int(views), "adds": int(adds), "orders": int(orders_events)},
    }
